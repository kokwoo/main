...
To redo an undo operation, use the command `redo`. Note that the redo operation can only be used after an undo command has been used. Multiple redo's are supported.
